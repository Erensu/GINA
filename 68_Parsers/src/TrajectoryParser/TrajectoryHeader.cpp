...
namespace GINAParser {
	...
	const string TrajectoryHeader::secondLineOfGINATrajectoryTag = "TYPE GINA TRAJECTORY";
	...
	bool TrajectoryHeader::hasStartofHeaderFound(string& line, gpstk::FFStream& ffs) {
		...
		if (firstLineisRead == false) {
			...
			if (valid == false && line != startofHeaderGinaTag && line_second != secondLineOfGINATrajectoryTag) {
				valid = false;
			}

			if (line == startofHeaderGinaTag && line_second == secondLineOfGINATrajectoryTag)
			{
				valid = true;
				strm.header = *this;
	...
	bool TrajectoryHeader::writeStartofHeader( gpstk::FFStream& ffs) const {
		...
		if (strm.lineNumber == 0) {
			...
			strm << secondLineOfGINATrajectoryTag << endl;
			strm.lineNumber++;
			return true;
		}
		else {
